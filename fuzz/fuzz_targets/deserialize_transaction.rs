...
fn do_test(data: &[u8]) {
    let tx_result: Result<fujicoin::blockdata::transaction::Transaction, _> = fujicoin::consensus::encode::deserialize(data);
    match tx_result {
        Err(_) => {},
        Ok(mut tx) => {
            let ser = fujicoin::consensus::encode::serialize(&tx);
            assert_eq!(&ser[..], data);
            let len = ser.len();
            ...
            let no_witness_len = fujicoin::consensus::encode::serialize(&tx).len();
