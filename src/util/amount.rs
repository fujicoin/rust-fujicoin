...
mod tests {
    ...
    fn parsing() {
        ...
        // exactly 50 chars.
        assert_eq!(p("100000000000000.0000000000000000000000000000000000", Denomination::Fujicoin), Err(E::TooBig));
        // more than 50 chars.
        assert_eq!(p("100000000000000.00000000000000000000000000000000000", Denomination::Fujicoin), Err(E::InputTooLarge));
    }
    ...
    fn to_from_string_in() {
        ...
        assert_eq!("0.00253583", Amount::from_sat(253583).to_string_in(D::Fujicoin));
        assert_eq!("-5", SignedAmount::from_sat(-5).to_string_in(D::Satoshi));
        assert_eq!("0.10000000", Amount::from_sat(100_000_00).to_string_in(D::Fujicoin));
        ...
        assert_eq!(ua_str(&ua_sat(500).to_string_in(D::Fujicoin), D::Fujicoin), Ok(ua_sat(500)));
        assert_eq!(ua_str(&ua_sat(21_000_000).to_string_in(D::Bit), D::Bit), Ok(ua_sat(21_000_000)));
        assert_eq!(ua_str(&ua_sat(1).to_string_in(D::MicroFujicoin), D::MicroFujicoin), Ok(ua_sat(1)));
        assert_eq!(ua_str(&ua_sat(1_000_000_000_000).to_string_in(D::MilliFujicoin), D::MilliFujicoin), Ok(ua_sat(1_000_000_000_000)));
        assert_eq!(ua_str(&ua_sat(u64::max_value()).to_string_in(D::MilliFujicoin), D::MilliFujicoin),  Err(ParseAmountError::TooBig));

        assert_eq!(sa_str(&sa_sat(-1).to_string_in(D::MicroFujicoin), D::MicroFujicoin), Ok(sa_sat(-1)));

        assert_eq!(sa_str(&sa_sat(i64::max_value()).to_string_in(D::Satoshi), D::MicroFujicoin), Err(ParseAmountError::TooBig));
        // Test an overflow bug in `abs()`
        assert_eq!(sa_str(&sa_sat(i64::min_value()).to_string_in(D::Satoshi), D::MicroFujicoin), Err(ParseAmountError::TooBig));
    ...
    fn to_string_with_denomination_from_str_roundtrip() {
        ...
        assert_eq!(Amount::from_str("42 satoshi FJC"), Err(ParseAmountError::InvalidFormat));
        assert_eq!(SignedAmount::from_str("-42 satoshi FJC"), Err(ParseAmountError::InvalidFormat));
    }
