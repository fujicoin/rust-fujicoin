...
pub struct Params {
    ...
    ///
    /// Note that this value differs from Fujicoin Core's powLimit field in that this value is
    /// attainable, but Fujicoin Core's is not. Specifically, because targets in Fujicoin are always
    ...
    /// this is used in Fujicoin Core's consensus algorithm is in comparison and there are no
    /// compact-expressible values between Fujicoin Core's and the limit expressed here.
