...
mod test {
    ...
    // Fujicoin Core node v0.17.1 with Wireshark
    ...
    fn serve_tcp(pieces: Vec<Vec<u8>>) -> (JoinHandle<()>, BufReader<TcpStream>) {
        // 1. Creating server part (emulating Fujicoin Core node)
    ...
    fn read_sequencemsg_test() {
        // Setting up TCP connection emulation
        let (handle, istream) = serve_tcp(vec![
            // Real-world Fujicoin core communication case for /Satoshi:0.17.1/
